...
$nama_bulan = date("F Y", mktime(0, 0, 0, $bulan, 1, $tahun));
$html = '
<!DOCTYPE html>
<html>
<head>
...
<body>
    <h2>Rekap Kehadiran (' . $nama_bulan . ')</h2>
    <table>
        <tr>
            <th>No</th>
            <th>NIP</th>
...
while ($row = mysqli_fetch_assoc($query)) {
    // Salin logika perhitungan status yang akurat dari rekap.php
    ...
    $html .= '
        <tr class="' . $isLateClass . '">
            ...
            <td>' . (!empty($row['jam_mulai']) ? substr($row['jam_mulai'], 0, 5) . ' - ' . substr($row['jam_selai'], 0, 5) : '-') . '</td>
            <td>' . $status . '</td>
        </tr>';
}
...
$mpdf->WriteHTML($html);
$mpdf->Output('rekap_kehadiran_' . $bulan . '_' . $tahun . '.pdf', 'I'); // 'I' untuk tampil di browser
