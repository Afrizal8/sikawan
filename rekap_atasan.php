...
// --- LOGIKA BARU UNTUK FILTER BULANAN ---
// Ambil bulan dan tahun dari URL, atau gunakan bulan dan tahun saat ini sebagai default
$bulan = $_GET['bulan'] ?? date('m');
$tahun = $_GET['tahun'] ?? date('Y');
$filter_shift = $_GET['shift'] ?? '';

// --- QUERY SQL BARU UNTUK FILTER BULANAN ---
// Query diubah untuk menggunakan MONTH() dan YEAR()
$query = mysqli_query($conn, "
    ...
    WHERE MONTH(h.tanggal_masuk) = '$bulan'
    AND YEAR(h.tanggal_masuk) = '$tahun'
...
<body>
<div class="container">
    <h2>📊 Rekap Kehadiran (<?= date("F Y", mktime(0, 0, 0, $bulan, 1, $tahun)) ?>)</h2>
        ...
        <form method="get">
            ...
            <select name="bulan">
                <?php for ($i = 1; $i <= 12; $i++): $val = str_pad($i, 2, '0', STR_PAD_LEFT); ?>
                    <option value="<?= $val ?>" <?= ($bulan == $val) ? 'selected' : '' ?>><?= date("F", mktime(0,0,0,$i,1)) ?></option>
                <?php endfor; ?>
            </select>
    ...
    <table>
        ...
        while ($row = mysqli_fetch_assoc($query)):
            // Logika status keterlambatan tetap sama
    ...
    <div class="tombol-container">
        <a href="export_rekappdf_atasan.php?bulan=<?= htmlspecialchars($bulan) ?>&tahun=<?= htmlspecialchars($tahun) ?>&shift=<?= htmlspecialchars($filter_shift) ?>" target="_blank" class="tombol pdf">
            📄 Cetak PDF
        </a>
        <a href="export_rekapexcel_atasan.php?bulan=<?= htmlspecialchars($bulan) ?>&tahun=<?= htmlspecialchars($tahun) ?>&shift=<?= htmlspecialchars($filter_shift) ?>" target="_blank" class="tombol excel">
            📊 Cetak Excel
        </a>
    </div>
